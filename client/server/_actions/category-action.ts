...
export const useGetCategory = () => {
  const queryFn = async (): Promise<ICategory[]> => {
    const response = await axiosInstance.get<IBaseResponse<ICategory[]>>(
      CATEGORY_PATHS.GET_ALL
    );
...
> = () => {
  ...
  >({
    mutationFn: (body: { id: number; name: string; description: string }) =>
      axiosInstance.patch<IBaseResponse<any>>(
        `${CATEGORY_PATHS.UPDATE}/${body.id}`,
        body
      ),
    onMutate: () => {
      toast({
...
> = () => {
  return useMutation<IBaseResponse<any>, Error, { id: number }>({
    mutationFn: (body: { id: number }) =>
      axiosInstance.delete<IBaseResponse<any>>(
        `${CATEGORY_PATHS.DELETE}/${body.id}`
      ),
    onMutate: () => {
      toast({
        variant: "destructive",
