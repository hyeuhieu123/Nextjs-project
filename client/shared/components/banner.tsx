...
const Banner = (props: PropsBanner) => {
  ...
  return (
    <div
      className={cn(
        "mt-10 md:mt-0 relative flex items-center justify-between w-screen h-[250px] md:h-[450px] lg:h-[650px]",
        className
      )}
    >
      <div className="flex flex-col items-start justify-center w-full h-full gap-5 px-4 mx-auto tracking-wide text-white md:px-8">
        <div
          className={cn(
            "mt-2 text-2xl font-semibold leading-5 md:mt-5 md:text-4xl md:leading-10 capitalize text-black bg-white p-2 rounded-sm"
          )}
        >
          {data.title}
        </div>
      ...
      <div className="absolute top-0 left-0 w-full h-full -z-10">
        <Image
          ...
          priority={true}
          className={cn(
            "object-cover w-full h-full object-top md:object-center"
          )}
        />
      </div>
    </div>
