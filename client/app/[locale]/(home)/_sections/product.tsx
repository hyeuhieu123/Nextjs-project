...
const ProductSection = () => {
  ...
  const [currentTab, setCurrentTab] = useState(
    categories && categories[0]!.name.toLowerCase()
  );
  ...
  return (
    <section className="container w-full min-h-[250px] md:min-h-[350px] h-full my-5 md:my-10 mx-20">
      {categories && (
        <Tabs
          value={currentTab}
        ...
        >
          <TabsList className="w-full h-full space-x-2  bg-transparent flex justify-start">
            {categories.map((item, i) => {
              return (
                <TabsTrigger
                  key={i}
                  value={item.name.toLowerCase()}
                  className="rounded-none py-3 px-2 text-black dark:text-white text-base  border border-black/50 data-[state=active]:bg-black dark:data-[state=active]:bg-white data-[state=active]:text-white dark:data-[state=active]:text-black font-bold"
                >
                  {item.name}
                </TabsTrigger>
              );
          ...
          >
            {products
              ?.filter(
                (item) => item.category.name.toLowerCase() === currentTab
              )
              ?.map((item, i) => {
                return (
                  <Card
                  ...
                  >
                    ...
                    <CardDescription className="flex flex-col items-start justify-start w-full gap-2 px-3 py-2">
                      <h2 className="text-base text-black dark:text-white md:text-xl">
                        {item.name}
                      </h2>
                      <p className="text-xs text-neutral-400 md:text-sm text-wrap">
                        {item.description}
                      </p>
                    </CardDescription>
                  </Card>
